...
def get_data():
    ...
    SELECT *
    FROM {project_name} t
    WHERE {primary_date} >= %(start_date)s
    AND {primary_date} <= %(end_date)s
    ORDER BY {primary_date}
    """.format(**dict(primary_date=data_conf.primary_date, project_name=proj_conf.project_name))
...
ORDER BY SUM DESC;""".format(data_conf.start_date, data_conf.end_date)
        ...
        sqlstr = """
            SELECT a.{field_name}, fk_tbl.{field_name}, count(a.{field_name})
            FROM {project_name} a, {field_name}_tbl fk_tbl
            WHERE {primary_date} >= '{start_date}'
            AND {primary_date} <= '{end_date}'
            AND fk_tbl.id = a.{field_name}
            GROUP BY a.{field_name}, fk_tbl.{field_name}
            ORDER BY count(a.{field_name}) {order}
        """

        templ_vals = dict(field_name=field_name, project_name=proj_conf.project_name,
                start_date=data_conf.start_date, end_date=data_conf.end_date, order=order,
                primary_date=data_conf.primary_date)
        sqlstr = sqlstr.format(**templ_vals)
        print(sqlstr)

...
column_names = list(set([i[0] for i in columns])) #sometimes there's overlap of date/nondate
...
def get_timeframe():
    ...
    for date_col in date_columns:
        sqlstr = """
          SELECT MIN({}) date_col 
          FROM {}
          WHERE {} > '1900-01-01'""".format(date_col, proj_conf.project_name, date_col)
        ...
        sqlstr = """
          SELECT MAX({}) date_col 
          FROM {}
          WHERE {} < '2150-01-01'""".format(date_col, proj_conf.project_name, date_col)
...
def get_useful_and_useless_cols(column_names, cutoff_count=200):
    ...
    return list(set(useful_cols)), list(set(useless_cols))
...
def get_x_y_cols(x_min=-125.0011, x_max=-66.9326, y_min=24.9493, y_max=49.5904, min_ratio=.9):
    ...
    sql_templ = """
      ...
      FROM information_schema.columns
      WHERE table_name = '{}'
      AND data_type in ('double precision', 'real')""".format(proj_conf.project_name)
...
for col_name in useful_cols:
    if col_name in lng_cols or col_name in lat_cols or col_name == primary_date:
        continue
...
multi_selectors.append({'column_name': 'neighborhood', 'title': 'Neighborhood'})
...
geo_files = [
    ...
    dict(path="data/neighborhood.geojson", bounds_key='community', proj_key='neighborhood', name='Community', table='neighborhood_geo'),
]
